...
const BASE_DOMAIN = 'phonetastic.ai';
...
export class ResendDomainServiceImpl implements ResendDomainService {
  ...
  async createDomain(subdomain: string): Promise<{ id: string; records: DnsRecord[] }> {
    const { data, error } = await this.client.domains.create({ name: `${subdomain}.${BASE_DOMAIN}` });
...
export class StubResendDomainService implements ResendDomainService {
  ...
  async createDomain(subdomain: string): Promise<{ id: string; records: DnsRecord[] }> {
    ...
    const records: DnsRecord[] = [
      { type: 'MX', name: `${subdomain}.${BASE_DOMAIN}`, value: 'feedback-smtp.us-east-1.amazonses.com', priority: 10 },
      { type: 'TXT', name: `${subdomain}.${BASE_DOMAIN}`, value: 'v=spf1 include:amazonses.com ~all' },
    ];
